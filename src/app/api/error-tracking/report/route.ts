...
interface ErrorStats {
  ...
  low_errors: number
  client_impacted: number
  unresolved: number
  avg_cost: number
}

function getDateRange(timeframe: string): TimeConfig {
  switch (timeframe) {
    case 'week':
...
function calculateStats(periodErrors: any[]): ErrorStats {
  return {
...
function groupErrorsByEmployee(periodErrors: any[]) {
  return (periodErrors || []).reduce((acc: any, error: any) => {
...
function groupErrorsByType(periodErrors: any[]) {
  return (periodErrors || []).reduce((acc: any, error) => {
...
function generateTrendAnalysis(periodErrors: any[], timeConfig: TimeConfig) {
  if (timeConfig.days < 30) return null

  const weeks = []
  const weekCount = Math.min(Math.floor(timeConfig.days / 7), 8)
...
function generateHtmlReport(
  ...
  if (trendAnalysis) {
    ...
    trendAnalysis.forEach((week: any) => {
      htmlTemplate.push(
  ...
  if (Object.keys(errorsByEmployee).length > 0) {
    htmlTemplate.push(
      '      <table class="table">',
      '        <thead>',
      '          <tr>',
    ...
    htmlTemplate.push('      <div class="no-errors">🎉 Nessun errore registrato per questo periodo!</div>')
  }

  htmlTemplate.push('    </div>')

  // Add error type analysis section
  htmlTemplate.push(
    '    <div class="section">',
  ...
  if (Object.keys(errorsByType).length > 0) {
    ...
    htmlTemplate.push('      <div class="no-errors">🎉 Nessun errore per tipo!</div>')
  }
  ...
  if (periodErrors && periodErrors.length > 0) {
    periodErrors.slice(0, 50).forEach((error: any) => {
      htmlTemplate.push(
    ...
    htmlTemplate.push('        <div class="no-errors">🎉 Nessun errore registrato per questo periodo! Eccellente lavoro del team!</div>')
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Calculate statistics and groupings
    const stats = calculateStats(periodErrors || [])
    const errorsByEmployee = groupErrorsByEmployee(periodErrors || [])
    const errorsByType = groupErrorsByType(periodErrors || [])
    const trendAnalysis = generateTrendAnalysis(periodErrors || [], timeConfig)
